...
import com.fator3.moov.models.Route;
...
public class PropertyIntegrationTest extends AbstractIntegrationTest {
    ...
    public void filterShouldReturnTwo() {
        ...
        final List<PersistentProperty> filterResult = Arrays.asList(RestAssured.given()
                .contentType("application/json").body(references).when().post(FILTER_URL).then()
                .statusCode(200).and().extract().response().as(PersistentProperty[].class));
