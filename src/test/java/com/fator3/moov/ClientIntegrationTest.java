...
public class ClientIntegrationTest extends AbstractIntegrationTest {
	...
	public void clientShouldAdjustCenterAndReturnFiftyBoundaries() {
		...
		assertThat(reachableResponse.getReachableRange().getBoundary().size()).isEqualTo(expectedBoundarySize);
	}
	...
	public void clientShouldReturnRouteWithRightLegsQuantityAndDistanceInSeconds() {
		...
		final List<TimedLatLng> locations = Lists.newArrayList(TimedLatLng.of(first), TimedLatLng.of(second),
				TimedLatLng.of(first));
